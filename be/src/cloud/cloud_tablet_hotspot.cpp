...
void TabletHotspot::count(const TabletSharedPtr& tablet) {
...
using MapKey = std::pair<int64_t, int64_t>;

void TabletHotspot::get_top_n_hot_partition(std::vector<THotTableMessage>* hot_tables) {
    std::unordered_map<MapKey, std::unordered_map<int64_t, MapValue>, MapKeyHash>
            day_hot_partitions;
    std::unordered_map<MapKey, std::unordered_map<int64_t, MapValue>, MapKeyHash>
            week_hot_partitions;
    ...
    auto get_return_partitions =
            [=, &return_partitions](
                    const std::unordered_map<MapKey, std::unordered_map<int64_t, MapValue>,
                                             MapKeyHash>& hot_partition) {
                for (auto& [key, partition_to_value] : hot_partition) {
