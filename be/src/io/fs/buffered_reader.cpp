...
void PrefetchBuffer::reset_offset(size_t offset) {
    {
        ...
        if (!_prefetched.wait_for(lck, std::chrono::milliseconds(config::buffered_reader_read_timeout_ms),
...
void PrefetchBuffer::prefetch_buffer() {
    {
        std::unique_lock lck {_lock};
        if (!_prefetched.wait_for(lck, std::chrono::milliseconds(config::buffered_reader_read_timeout_ms), [this]() {
