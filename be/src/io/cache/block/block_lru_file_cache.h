 ...
 */
class LRUFileCache final : public BlockFileCache {
public:
    /**
    ...
    size_t get_used_cache_size(FileCacheType type) const override;

    size_t get_file_segments_num(FileCacheType type) const override;
...
private:
    ...
    LRUFileCache::LRUQueue& get_queue(FileCacheType type);
    const LRUFileCache::LRUQueue& get_queue(FileCacheType type) const;
    ...
    size_t get_file_segments_num_unlocked(FileCacheType type,
                                          std::lock_guard<std::mutex>& cache_lock) const;
