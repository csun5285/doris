...
LRUFileCache::LRUFileCache(const std::string& cache_base_path,
        ...
        : BlockFileCache(cache_base_path, cache_settings) {
...
bool LRUFileCache::need_to_move(FileCacheType cell_type, FileCacheType query_type) const {
    return query_type != FileCacheType::DISPOSABLE || cell_type == FileCacheType::DISPOSABLE ? true : false;
}
...
LRUFileCache::LRUQueue& LRUFileCache::get_queue(FileCacheType type) {
    switch (type) {
...
const LRUFileCache::LRUQueue& LRUFileCache::get_queue(FileCacheType type) const {
    switch (type) {
    case FileCacheType::INDEX:
        return _index_queue;
    case FileCacheType::DISPOSABLE:
...
std::vector<FileCacheType> LRUFileCache::get_other_cache_type(FileCacheType cur_cache_type) {
...
bool LRUFileCache::try_reserve_from_other_queue(FileCacheType cur_cache_type, size_t size,
                                                int64_t cur_time,
                                                std::lock_guard<std::mutex>& cache_lock) {
...
size_t LRUFileCache::get_used_cache_size(FileCacheType cache_type) const {
    std::lock_guard cache_lock(_mutex);
    return get_used_cache_size_unlocked(cache_type, cache_lock);
}

size_t LRUFileCache::get_used_cache_size_unlocked(FileCacheType cache_type,
                                                  std::lock_guard<std::mutex>& cache_lock) const {
    return get_queue(cache_type).get_total_cache_size(cache_lock);
}

size_t LRUFileCache::get_available_cache_size(FileCacheType cache_type) const {
    std::lock_guard cache_lock(_mutex);
...
size_t LRUFileCache::get_file_segments_num(FileCacheType cache_type) const {
    std::lock_guard cache_lock(_mutex);
    return get_file_segments_num_unlocked(cache_type, cache_lock);
}

size_t LRUFileCache::get_file_segments_num_unlocked(FileCacheType cache_type,
                                                    std::lock_guard<std::mutex>& cache_lock) const {
    return get_queue(cache_type).get_elements_num(cache_lock);
}

LRUFileCache::FileBlockCell::FileBlockCell(FileBlockSPtr file_block, FileCacheType cache_type,
                                           std::lock_guard<std::mutex>& cache_lock)
...
BlockFileCache::LRUQueue::Iterator BlockFileCache::LRUQueue::add(
        const BlockFileCache::Key& key, size_t offset, size_t size,
        std::lock_guard<std::mutex>& /* cache_lock */) {
    cache_size += size;
    return queue.insert(queue.end(), FileKeyAndOffset(key, offset, size));
}

void BlockFileCache::LRUQueue::remove(Iterator queue_it,
                                  std::lock_guard<std::mutex>& /* cache_lock */) {
...
bool BlockFileCache::LRUQueue::contains(const BlockFileCache::Key& key, size_t offset,
                                    std::lock_guard<std::mutex>& /* cache_lock */) const {
...
std::string BlockFileCache::LRUQueue::to_string(std::lock_guard<std::mutex>& /* cache_lock */) const {
    std::string result;
