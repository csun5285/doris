...
public:
    ...
    VStatisticsIterator(std::shared_ptr<Segment> segment, DenseReadSchemaSPtr schema)
            : _segment(std::move(segment)), _schema(std::move(schema)) {}
...
private:
    ...
    std::unordered_map<DenseFieldIdentity, std::unique_ptr<ColumnIterator>, DenseFieldIdentityHash>
            _column_iterators_map;
...
public:
    VMergeIteratorContext(RowwiseIteratorUPtr&& iter, int sequence_id_idx, bool is_unique,
                          ...
                          DenseReadSchemaSPtr output_schema, bool small_seq_first = false)
            : _iter(std::move(iter)),
        ...
        _key_positions.reserve(_output_schema->key_positions().size());
        for (ReadPosition position : _output_schema->key_positions()) {
    ...
    // Reset (or initialize) the internal block using the exact output schema.
    // Inputs must already have been projected to this same layout.
    ...
    // Copy rows from internal _block to a destination with the same exact schema.
    //
...
public:
    ...
    VMergeIterator(std::vector<RowwiseIteratorUPtr>&& iters, int sequence_id_idx, bool is_unique,
                   bool is_reverse, uint64_t* merged_rows, DenseReadSchemaSPtr output_schema,
...
// Project a child iterator to target_schema by stable field identity. This is
// the explicit boundary between a SegmentSchema Block and a ScanSchema Block.
