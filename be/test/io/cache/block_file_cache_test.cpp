...
public:
    static void SetUpTestSuite() {
        ...
        config::enable_file_cache_block_prefetch = true;
        bool exists {false};
...
void test_file_cache(io::FileCacheType cache_type) {
    ...
    auto key = io::BlockFileCache::hash("key1");
    {
        ...
        ASSERT_EQ(cache.get_file_blocks_num(cache_type), 9);
        // Current cache:    [__________][_____][_][____][_]  [_][___][_]    [__]
        //                   ^          ^^     ^   ^    ^        ^   ^  ^    ^  ^
        //                   0          910    14  17   20       24  26 27   30 31
        {
            ...
            assert_range(39, segments[5], io::FileBlock::Range(22, 22),
                         io::FileBlock::State::EMPTY);
            ASSERT_TRUE(segments[5]->get_or_set_downloader() == io::FileBlock::get_caller_id());
            ASSERT_TRUE(segments[5]->state() == io::FileBlock::State::DOWNLOADING);
            ...
            std::thread other_1([&] {
                ...
                assert_range(41, segments_2[0], io::FileBlock::Range(0, 9),
                             io::FileBlock::State::DOWNLOADED);
                assert_range(42, segments_2[5], io::FileBlock::Range(22, 22),
                             io::FileBlock::State::DOWNLOADING);

                ASSERT_TRUE(segments_2[5]->get_downloader() != io::FileBlock::get_caller_id());
                ASSERT_TRUE(segments_2[5]->state() == io::FileBlock::State::DOWNLOADING);
                ...
                while (segments_2[5]->wait() == io::FileBlock::State::DOWNLOADING) {
                }
                ASSERT_TRUE(segments_2[5]->state() == io::FileBlock::State::EMPTY);
                ASSERT_TRUE(segments_2[5]->get_or_set_downloader() ==
                            io::FileBlock::get_caller_id());
                download(segments_2[5]);
            });
            ...
            holder.reset();
            other_1.join();
            ASSERT_TRUE(segments[5]->state() == io::FileBlock::State::DOWNLOADED);
        }
    }
    ...
    //                   0          9  17   20       24  26 27   30 31
    {
        ...
        assert_range(44, segments1[0], io::FileBlock::Range(0, 9),
                     io::FileBlock::State::DOWNLOADED);
        assert_range(45, segments1[1], io::FileBlock::Range(10, 14), io::FileBlock::State::EMPTY);
        assert_range(45, segments1[2], io::FileBlock::Range(15, 16),
                     io::FileBlock::State::DOWNLOADED);
        assert_range(45, segments1[3], io::FileBlock::Range(17, 20),
                     io::FileBlock::State::DOWNLOADED);
        assert_range(46, segments1[4], io::FileBlock::Range(21, 21),
                     io::FileBlock::State::DOWNLOADED);
    }
    {
...
void test_file_cache_run_in_resource_limit(io::FileCacheType cache_type) {
    ...
    index_context.cache_type = io::FileCacheType::INDEX;
    {
        ...
        ASSERT_EQ(cache.get_used_cache_size(io::FileCacheType::INDEX), 60);
        {
            ...
            /// Range was not present in cache. It should be added in cache as one while file segment.
            ASSERT_EQ(segments.size(), 1);
            assert_range(3, segments[0], io::FileBlock::Range(0, 9), io::FileBlock::State::EMPTY);
            ASSERT_TRUE(segments[0]->get_or_set_downloader() == io::FileBlock::get_caller_id());
            assert_range(4, segments[0], io::FileBlock::Range(0, 9),
                         io::FileBlock::State::DOWNLOADING);
            download(segments[0]);
...
TEST_F(BlockFileCacheTest, test_lazy_load) {
    ...
    sp->set_call_back("CloudFileCache::TmpFile2", [&](auto&&) {
        while (!flag1) {
        }
    });
    io::BlockFileCache cache(cache_base_path, settings);
...
TEST_F(BlockFileCacheTest, ttl_normal) {
    ...
    std::this_thread::sleep_for(std::chrono::seconds(10));
    {
        ...
        ASSERT_TRUE(segments[0]->read_at(Slice(buffer.data(), 10), 0).ok());
...
TEST_F(BlockFileCacheTest, ttl_change_to_normal) {
    ...
    }
    {
        ...
        auto holder = cache.get_or_set(key2, 50, 10, context); /// Add range [50, 59]
        EXPECT_TRUE(
                fs::exists(cache.get_path_in_local_cache(key2, 0, 50, io::FileCacheType::NORMAL)));
        auto segments = fromHolder(holder);
        ASSERT_EQ(segments.size(), 1);
        assert_range(1, segments[0], io::FileBlock::Range(50, 59),
                     io::FileBlock::State::DOWNLOADED);
...
TEST_F(BlockFileCacheTest, ttl_change_expiration_time) {
    ...
    }
    {
        ...
        EXPECT_TRUE(fs::exists(
                cache.get_path_in_local_cache(key2, change_time, 50, io::FileCacheType::TTL)));
        auto segments = fromHolder(holder);
        ASSERT_EQ(segments.size(), 1);
        assert_range(1, segments[0], io::FileBlock::Range(50, 59),
                     io::FileBlock::State::DOWNLOADED);
...
TEST_F(BlockFileCacheTest, ttl_reverse) {
    ...
    {
        auto holder = cache.get_or_set(key2, 50, 7, context); /// Add range [50, 54]
        auto blocks = fromHolder(holder);
...
TEST_F(BlockFileCacheTest, append_many_time) {
    ...
    }
    {
        ...
        {
            segments[0]->change_cache_type_self(FileCacheType::NORMAL);
            EXPECT_EQ(segments[0]->cache_type(), FileCacheType::INDEX);
            std::string buffer;
            buffer.resize(5);
            EXPECT_TRUE(segments[0]->read_at(Slice(buffer.data(), 5), 0).ok());
            EXPECT_EQ(buffer, std::string(5, '0'));
        }
        {
            EXPECT_FALSE(segments[0]->change_cache_type(FileCacheType::NORMAL));
            EXPECT_EQ(segments[0]->cache_type(), FileCacheType::INDEX);
            std::string buffer;
            buffer.resize(5);
            EXPECT_TRUE(segments[0]->read_at(Slice(buffer.data(), 5), 0).ok());
...
TEST_F(BlockFileCacheTest, cached_remote_file_reader_tail) {
    ...
    EXPECT_TRUE(reader.closed());
    {
        ...
        auto segments = fromHolder(holder);
        ASSERT_EQ(segments.size(), 2);
        assert_range(1, segments[0], io::FileBlock::Range(9_mb, 10_mb - 1),
                     io::FileBlock::State::DOWNLOADED);
        assert_range(2, segments[1], io::FileBlock::Range(10_mb, 10_mb),
                     io::FileBlock::State::DOWNLOADED);
    }
    if (fs::exists(cache_base_path)) {
        fs::remove_all(cache_base_path);
...
TEST_F(BlockFileCacheTest, cached_remote_file_reader_init) {
    ...
    opts.metrics_hook = count;
    {
        ...
        auto key = io::BlockFileCache::hash(
                fmt::format("{}:{}", local_reader->path().native(), opts.modification_time));
        EXPECT_EQ(reader._cache_key, key);
    }
    {
...
TEST_F(BlockFileCacheTest, test_lazy_load_with_error_file_1) {
    ...
    auto key = io::BlockFileCache::hash("key1");
    io::BlockFileCache cache(cache_base_path, settings);
    std::string dir = cache.get_path_in_local_cache(key, 0);
    sp->set_call_back("CloudFileCache::TmpFile1", [&](auto&&) {
        FileWriterPtr writer;
