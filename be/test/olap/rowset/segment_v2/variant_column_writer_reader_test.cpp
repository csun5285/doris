...
TEST_F(VariantColumnWriterReaderTest, test_legacy_flat_dot_key_reader_init) {
    ...
    const int kNumRows = 8;
    {
        ...
        for (int i = 0; i < kNumRows; ++i) {
            std::string json = R"({"a": {"b": "v)" + std::to_string(i) + R"("}})";
            json_col->insert_data(json.data(), json.size());
        }
        ParseConfig config;
        config.enable_flatten_nested = false;
        parse_json_to_variant(*column_object, *json_col, config);
        ...
        olap_data_convertor->set_source_content(&block, 0, kNumRows);
    ...
    segment_v2::ColumnReaderOptions read_opts;
    read_opts.tablet_schema = _tablet_schema;
    std::shared_ptr<segment_v2::ColumnReader> column_reader;
    st = segment_v2::ColumnReader::create(read_opts, footer, 0, kNumRows, file_reader,
                                          &column_reader);
    ASSERT_TRUE(st.ok()) << st.msg();
