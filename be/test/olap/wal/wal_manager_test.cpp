...
TEST_F(WalManagerTest, TestRollUp) {
    _env->wal_mgr()->wal_limit_test_bytes = 1099511627776;
    config::group_commit_replay_wal_retry_interval_seconds = 1;
    config::group_commit_scan_other_be_wal_interval_seconds = 2;
    config::group_commit_replay_other_be_wal_time_diff_seconds = 2;
    std::string db_1_id = "1";
    int64_t tb_1_id = 1;
    std::string own_be_wal_file = "0_1001_5_group_commit_label5";
    std::string other_be_wal_file = "0_1003_6_group_commit_label6";
    bool res = std::filesystem::create_directory(wal_dir.string() + "/" + db_1_id);
    ASSERT_TRUE(res);
    res = std::filesystem::create_directory(wal_dir.string() + "/" + db_1_id + "/" +
                                            std::to_string(tb_1_id));
    ASSERT_TRUE(res);
    std::string wal_own = wal_dir.string() + "/" + db_1_id + "/" + std::to_string(tb_1_id) + "/" +
                          own_be_wal_file;
    createWal(wal_own);
    std::string wal_other = wal_dir.string() + "/" + db_1_id + "/" + std::to_string(tb_1_id) + "/" +
                            other_be_wal_file;
    createWal(wal_other);
    Status st = _env->wal_mgr()->init();
    if (!st.ok()) {
    ...
    auto count = 0;
    while (std::filesystem::exists(wal_own) || std::filesystem::exists(wal_other)) {
        if (count > 30) {
            LOG(WARNING) << "wait time out";
            break;
        }
    ...
    ASSERT_TRUE(!std::filesystem::exists(wal_own));
    ASSERT_TRUE(!std::filesystem::exists(wal_other));
}
