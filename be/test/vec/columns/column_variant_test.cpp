...
TEST_F(ColumnObjectTest, clone_finalized) {
    auto test_func = [](const auto& source_column) {
        ...
        const auto& cloned_subcolumns = assert_cast<ColumnObject*>(cloned.get())->get_subcolumns();
        if (cloned_subcolumns.size() != original_subcolumns.size()) {
            // subcolumns type is nothing type
            EXPECT_EQ(cloned_subcolumns.size(), 1);
            return;
        }
