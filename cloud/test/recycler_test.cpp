...
static std::unique_ptr<selectdb::RecyclerThreadPoolGroup> thread_group = nullptr;
...
int main(int argc, char** argv) {
    ...
    thread_group = std::make_unique<selectdb::RecyclerThreadPoolGroup>();
...
static doris::RowsetMetaPB create_rowset(const std::string& resource_id, int64_t tablet_id,
                                         int64_t index_id, int num_segments,
                                         const doris::TabletSchemaPB& schema, int64_t txn_id = 0) {
...
static int create_recycle_rowset(TxnKv* txn_kv, ObjStoreAccessor* accessor,
                                 const doris::RowsetMetaPB& rowset, RecycleRowsetPB::Type type,
...
TEST(RecyclerTest, recycle_copy_jobs) {
    ...
    InstanceRecycler recycler(txn_kv, instance_info, *thread_group.get());
    ASSERT_EQ(recycler.init(), 0);
...
TEST(RecyclerTest, delete_rowset_data) {
    ...
    }
    {
        ...
        InstanceRecycler recycler(txn_kv, tmp_instance, *thread_group.get());
        ASSERT_EQ(recycler.init(), 0);
        auto accessor = recycler.accessor_map_.begin()->second;
