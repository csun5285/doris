...
void MetaServiceImpl::begin_txn(::google::protobuf::RpcController* controller,
    ...
    int64_t txn_id = 0;
    ret = get_txn_id_from_fdb_ts(
            std::string_view(txn_label_val)
                    .substr(txn_label_val.size() - VERSION_STAMP_LEN, txn_label_val.size()),
            &txn_id);
    if (ret != 0) {
    ...
    if (txn_label_val.size() > VERSION_STAMP_LEN) {
        ...
        if (!txn_label_pb.ParseFromArray(txn_label_val.data(),
                                         txn_label_val.size() - VERSION_STAMP_LEN)) {
...
void MetaServiceImpl::abort_txn(::google::protobuf::RpcController* controller,
    ...
    if (txn_id > 0) {
        ...
        TxnLabelPB txn_label_pb;
        DCHECK(txn_label_val.size() > VERSION_STAMP_LEN);
        if (!txn_label_pb.ParseFromArray(txn_label_val.data(),
                                         txn_label_val.size() - VERSION_STAMP_LEN)) {
            code = MetaServiceCode::PROTOBUF_PARSE_ERR;
