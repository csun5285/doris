...
void MetaServiceImpl::begin_txn(::google::protobuf::RpcController* controller,
    ...
    LOG(INFO) << "txn->get label_key=" << hex(label_key) << " label=" << label << " ret=" << ret;
    ...
    int64_t txn_id = 0;
    ret = get_txn_id_from_fdb_ts(std::string_view(label_val).substr(
    ...
    if (label_val.size() > VERSION_STAMP_LEN) {
        ...
        for (auto& cur_txn_id : label_pb.txn_ids()) {
            ...
            ret = txn->get(cur_info_key, &cur_info_val);
            if (ret < 0) {
                code = MetaServiceCode::KV_TXN_GET_ERR;
...
void MetaServiceImpl::check_txn_conflict(::google::protobuf::RpcController* controller,
    ...
    do {
        ret = txn->get(begin_running_key, end_running_key, &it, true);
        if (ret != 0) {
            code = MetaServiceCode::KV_TXN_GET_ERR;
