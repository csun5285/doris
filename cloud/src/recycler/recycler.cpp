...
RecyclerThreadPoolGroup::RecyclerThreadPoolGroup() {
    s3_producer_pool = std::make_unique<SimpleThreadPool>(config::recycle_pool_parallelism);
    s3_producer_pool->start();
    recycle_tablet_pool = std::make_unique<SimpleThreadPool>(config::recycle_pool_parallelism);
...
Recycler::Recycler(std::shared_ptr<TxnKv> txn_kv)
        : txn_kv_(std::move(txn_kv)),
          _thread_pool_group(std::make_unique<RecyclerThreadPoolGroup>()) {
...
InstanceRecycler::InstanceRecycler(std::shared_ptr<TxnKv> txn_kv, const InstanceInfoPB& instance,
        ...
        : txn_kv_(std::move(txn_kv)),
          ...
          _thread_pool_group(thread_pool_group) {}
...
int InstanceRecycler::do_recycle() {
    ...
    if (instance_info_.status() == InstanceInfoPB::DELETED) {
        ...
        SyncExecutor<int> sync_executor(_thread_pool_group.group_recycle_function_pool.get(),
...
int InstanceRecycler::recycle_tablets(int64_t table_id, int64_t index_id, int64_t partition_id,
    ...
    SyncExecutor<TabletKeyPair> sync_executor(
            _thread_pool_group.recycle_tablet_pool.get(),
...
int InstanceRecycler::delete_rowset_data(const std::vector<doris::RowsetMetaPB>& rowsets) {
    ...
    SyncExecutor<int> concurrent_delete_executor(_thread_pool_group.s3_producer_pool.get(),
                                                 "delete_rowset_data",
                                                 [](const int& ret) { return ret != 0; });
    for (auto& [resource_id, file_paths] : resource_file_paths) {
        concurrent_delete_executor.add([&, rid = &resource_id, paths = &file_paths]() {
            auto& accessor = accessor_map_[*rid];
            DCHECK(accessor);
            return accessor->delete_objects(*paths, instance_id_);
        });
    }
    bool finished = true;
...
int InstanceRecycler::recycle_tablet(int64_t tablet_id) {
    ...
    SyncExecutor<int> concurrent_delete_executor(
            _thread_pool_group.s3_producer_pool.get(),
    ...
    for (auto& [_, accessor] : accessor_map_) {
        concurrent_delete_executor.add([&, accessor_ptr = &accessor]() {
            if ((*accessor_ptr)
                        ->delete_objects_by_prefix(tablet_path_prefix(tablet_id), instance_id_) !=
                0) {
                LOG(WARNING) << "failed to delete rowset data of tablet " << tablet_id
                             << " s3_path=" << (*accessor_ptr)->path();
                return -1;
            }
            return 0;
        });
