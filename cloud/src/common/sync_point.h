...
# define TEST_SYNC_POINT(x) selectdb::SyncPoint::get_instance()->process(x)
# define TEST_IDX_SYNC_POINT(x, index) \
  selectdb::SyncPoint::get_instance()->process(x + std::to_string(index))
# define TEST_SYNC_POINT_CALLBACK(x, y) \
  selectdb::SyncPoint::get_instance()->process(x, y)
# define INIT_SYNC_POINT_SINGLETONS() \
  (void)selectdb::SyncPoint::get_instance();
...
# define TEST_INJECTION_POINT(x)                                          \
  if (ENABLE_INJECTION_PIONT) {                                           \
    selectdb::SyncPoint::get_instance()->process(x);                         \
  }
