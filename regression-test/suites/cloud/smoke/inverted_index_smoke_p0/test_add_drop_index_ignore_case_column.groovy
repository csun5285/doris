...
suite("smoke_test_add_drop_index_with_ignore_case_column", "smoke"){
    if (context.config.cloudVersion != null && !context.config.cloudVersion.isEmpty()
        ...
        log.info("case: smoke_test_add_drop_index_with_ignore_case_column, cloud version ${context.config.cloudVersion} less than 3.0.0, skip".toString());
        return
    }
    // prepare test table
    def timeout = 60000
