...
suite("test_orc", "p0") {
     ...
     def filePrefix = "copy_into/orc"
     def tableName = "orc_test"
     ...
     def errorMsgs = [
                     "",
                     "errCode = 2, detailMessage = ", // errCode = 2, detailMessage = Unknown column 'not_exist' in 'table list'
     ...
     def do_insert_into = {table, targetColumns, selectColumns, stageName, prefix, whereExpr ->
         try {
             ...
             select $selectColumns from stage('stage_name'='${stageName}', 'file_pattern'='${prefix}/part.orc', 'format'='orc') 
             $whereExpr
             """
         } catch (Exception e) {
             return e.getMessage()
