...
suite("test_json", "p0") {
     ...
     def errorMsgs = [
                     ...
                     // success in copy into, 6 columns in table, 9 columns in file
                     "errCode = 2, detailMessage = Column count doesn't match value count",
                     "",
                     "errCode = 2, detailMessage = Unknown column 'not_exist' in 'table list'",
                     "",
                     "",
     ...
     def do_insert_into = {table, targetColumns, selectColumns, stageName, prefix, whereExpr ->
         try {
             ...
             select $selectColumns from stage('stage_name'='${stageName}', 'file_pattern'='${prefix}/part.json', 'format'='json', 'read_json_by_line'='true') 
             $whereExpr
             """
         } catch (Exception e) {
             return e.getMessage()
