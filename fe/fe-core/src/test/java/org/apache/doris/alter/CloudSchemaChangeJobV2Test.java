...
public class CloudSchemaChangeJobV2Test {
    private static final Logger LOG = LogManager.getLogger(CloudSchemaChangeJobV2Test.class);
    ...
    private static ColumnDef newCol = new ColumnDef("add_v", new TypeDef(ScalarType.createType(PrimitiveType.INT)),
            true, null, false, new DefaultValue(true, "1"), "");
    private static AddColumnClause addColumnClause = new AddColumnClause(newCol, new ColumnPosition("k2"), null, null);
    ...
    @Test
    public void testCloudSchemaChange() throws Exception {
        Set<Long> prepareIndexes = new HashSet<Long>();
        Set<Long> commitIndexes = new HashSet<Long>();
        Set<Long> dropIndexes = new HashSet<Long>();
        new MockUp<MetaServiceProxy>(MetaServiceProxy.class) {
            ...
            @Mock
            public SelectdbCloud.IndexResponse dropIndex(SelectdbCloud.IndexRequest request) {
                dropIndexes.addAll(request.getIndexIdsList());
                LOG.info("dropIndexes:{}", dropIndexes);
                IndexResponse.Builder indexResponseBuilder = IndexResponse.newBuilder();
                indexResponseBuilder.setStatus(SelectdbCloud.MetaServiceResponseStatus.newBuilder()
                        .setCode(MetaServiceCode.OK).setMsg("OK"));
                return indexResponseBuilder.build();
        ...
        Assert.assertNotEquals(dropIndexes, commitIndexes);
    }
}
