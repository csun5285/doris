...
public class RemoteTableSchemaMgr extends MasterDaemon {
    ...
    private List<Table> tablesHasVariantColumn = Lists.newArrayList();
    ...
    private void clearTables() {
        tablesHasVariantColumn.clear();
    }

    private void fetchAndUpdateAllTableSchmea() {
        if (tablesHasVariantColumn.isEmpty()) {
            return;
        }
        // id: table id
        Map<Long, List<Column>> idToSchmea = Maps.newHashMap();
        for (Table table : tablesHasVariantColumn) {
            ...
            if (remoteSchema == null || remoteSchema.isEmpty()) {
                LOG.debug("fetch remote table schema failed, table is: {}", table.getName());
            } else {
                idToSchmea.put(table.getId(), remoteSchema);
            }
        }
    ...
    private void initTables() {
        ...
        for (List<Table> tbs : tables) {
            for (Table table : tbs) {
                ...
                tablesHasVariantColumn.add(table);
            }
        }
    }
}
