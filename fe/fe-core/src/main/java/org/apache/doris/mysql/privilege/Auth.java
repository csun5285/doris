...
public class Auth implements Writable {
    ...
    private void upgradeToVersion116(UserPrivTable userPrivTable, CatalogPrivTable catalogPrivTable,
        ...
        initUser();
        if (Config.isCloudMode()) {
            upgradeCloudToVersion116(userPrivTable);
        } else {
            upgradeNotCloudToVersion116(userPrivTable);
        }
    ...
    private void upgradeCloudToVersion116(UserPrivTable userPrivTable) throws PatternMatcherException, DdlException {
        List<PrivEntry> userPrivTableEntries = userPrivTable.getEntries();
        // in cloud userId save in userPrivTable, so first upgrade userPrivTable
        for (PrivEntry privEntry : userPrivTableEntries) {
            String userId = userPrivTable.getUserIdByUser(privEntry.getOrigUser());
        ...
        for (Entry<String, UserProperty> entry : propertyMgr.propertyMap.entrySet()) {
            for (Entry<String, byte[]> userEntry : entry.getValue().getWhiteList().getPasswordMap().entrySet()) {
                ...
                if (Strings.isNullOrEmpty(userId)) {
                    ...
                    LOG.info("use propertyMap's user {} to generate userId {}", userName, userId);
                }
                //create user
                User user = userManager
    ...
    public void upgradeNotCloudToVersion116(UserPrivTable userPrivTable) throws PatternMatcherException, DdlException {
        for (Entry<String, UserProperty> entry : propertyMgr.propertyMap.entrySet()) {
            for (Entry<String, byte[]> userEntry : entry.getValue().getWhiteList().getPasswordMap().entrySet()) {
                //create user
                User user = userManager
